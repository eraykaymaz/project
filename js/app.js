...
let BRANDS=[],SELECTED=new Set(),brandPrefix="Hazır",TSOFT_OK_SUP_BY_BRAND=new Map();
...
const applySupplierUi=()=>{
  const go=$("go");if(go)ACTIVE_SUPPLIER===SUPPLIERS.AKALIN?(go.classList.add("wip"),go.title="Yapım Aşamasında"):(go.classList.remove("wip"),go.title="Listele");
...
function pickHMFrom(obj){
  ...
  if(iso){const m=iso.match(/T(\d{2}:\d{2})/);if(m)return m[1];const m2=iso.match(/\b(\d{2}:\d{2})\b/);if(m2)return m2[1]}
  return""
}
function pickDateFrom(obj){
  if(!obj)return{ymd:"",dmy:""};
  return{ymd:String(obj.ymd||obj.YMD||obj.date||obj.day||obj.gun||"").trim(),dmy:String(obj.dmy||obj.DMY||obj.dateText||obj.display||obj.tarih||"").trim()}
}
function getAideDailyPick(){
  ...
  if(todayExists){const hm=pickHMFrom(todayAide)||pickHMFrom(DAILY_META?.today)||"";return{exists:true,isToday:true,ymd:todayDate.ymd,dmy:todayDate.dmy,hm}}
...
function getTsoftDailyPick(){
  ...
  const todayDate=pickDateFrom(DAILY_META?.today),yestDate=pickDateFrom(DAILY_META?.yesterday);
  if(todayExists){const hm=pickHMFrom(todayTsoft)||pickHMFrom(DAILY_META?.today)||"";return{exists:true,isToday:true,ymd:todayDate.ymd,dmy:todayDate.dmy,hm}}
  if(yestExists)return{exists:true,isToday:false,ymd:yestDate.ymd,dmy:yestDate.dmy,hm:""};
  return{exists:false,isToday:false,ymd:"",dmy:"",hm:""}
}
function paintDailyUI(){
  const tBtn=$("tsoftDailyBtn"),aBtn=$("aideDailyBtn"),tPrev=$("tsoftPrev"),aPrev=$("aidePrev");

  const tPick=getTsoftDailyPick();
  const tLabel=tPick.exists?(tPick.isToday?(tPick.hm?`Bugün ${tPick.hm} Tarihli Veri`:"Bugün — Tarihli Veri"):(tPick.dmy?`${tPick.dmy} Tarihli Veri`:"—")):"—";
  const tSel=!!(tPick.ymd&&DAILY_SELECTED.tsoft&&DAILY_SELECTED.tsoft===tPick.ymd);
  if(tBtn){tBtn.disabled=!tPick.exists;tBtn.title=tPick.exists?tLabel:"—";tBtn.textContent=tSel?"Seçildi":tLabel;setBtnSel(tBtn,tSel)}

  const aidePick=getAideDailyPick();
  const aLabel=aidePick.exists?(aidePick.isToday?(aidePick.hm?`Bugün ${aidePick.hm} Tarihli Veri`:"Bugün — Tarihli Veri"):(aidePick.dmy?`${aidePick.dmy} Tarihli Veri`:"—")):"—";
  const aSel=!!(aidePick.ymd&&DAILY_SELECTED.aide&&DAILY_SELECTED.aide===aidePick.ymd);
  if(aBtn){aBtn.disabled=!aidePick.exists;aBtn.title=aidePick.exists?aLabel:"—";aBtn.textContent=aSel?"Seçildi":aLabel;setBtnSel(aBtn,aSel)}
...
async function refreshDailyMeta(){try{DAILY_META=await dailyMeta(API_BASE)}catch(e){console.warn("daily meta fail",e);DAILY_META=null}paintDailyUI()}
const closeModalByButton=btnId=>{const b=$(btnId);b&&b.click()};
function toggleDaily(kind){
  if(kind==="tsoft"){
    const pick=getTsoftDailyPick();if(!pick?.exists||!pick?.ymd)return;
    const was=DAILY_SELECTED.tsoft===pick.ymd;DAILY_SELECTED.tsoft=was?"":pick.ymd;paintDailyUI();
    if(!was){closeModalByButton("tsoftDismiss");!hasEverListed&&SELECTED.size>0&&setGuideStep("aide")}
    else !hasEverListed&&SELECTED.size>0&&setGuideStep("tsoft")
  }else if(kind==="aide"){
    const pick=getAideDailyPick();if(!pick?.exists||!pick?.ymd)return;
    const was=DAILY_SELECTED.aide===pick.ymd;DAILY_SELECTED.aide=was?"":pick.ymd;paintDailyUI();
    if(!was){closeModalByButton("depoClose");!hasEverListed&&SELECTED.size>0&&setGuideStep("list")}
    else !hasEverListed&&SELECTED.size>0&&setGuideStep("aide")
  }
}
...
function ensureSaveCredOrCancel(){
  ...
  const admin=prompt("Yetkili Şifre:");if(!admin)return false;
  const read=prompt("Bugün için okuma şifresi:");if(!read?.trim())return false;
  DAILY_SAVE_CRED={adminPassword:String(admin).trim(),readPassword:String(read).trim()};
  return true
}
async function getReadPassOrPrompt(dateYmd){
  const ymd=String(dateYmd||"").trim();if(!ymd)throw new Error("Tarih bulunamadı");
  if(DAILY_READ_CACHE.pass&&DAILY_READ_CACHE.date===ymd)return DAILY_READ_CACHE.pass;
  const p=prompt("Seçilen günün verisini kullanmak için okuma şifresi:")||"";if(!p.trim())throw new Error("Şifre girilmedi");
  return p.trim()
}
...
(()=>{const box=$("sescBox"),inp=$("f2"),modal=$("tsoftModal"),inner=$("tsoftInner"),pick=$("tsoftClose"),dismiss=$("tsoftDismiss");
  ...
  const cb=$("tsoftSaveToday");cb&&cb.addEventListener("change",()=>{cb.checked&&!ensureSaveCredOrCancel()&&(cb.checked=false)})
})();
...
(()=>{const wrap=$("supplierWrap"),btn=$("supplierBtn"),menu=$("supplierMenu"),addBtn=$("supplierAddBtn"),itC=$("supplierCompelItem"),itA=$("supplierAkalinItem");
  ...
  const setSupplier=async name=>{
    ...
    if(name===SUPPLIERS.AKALIN){brandPrefix="Akalın";BRANDS=AKALIN_BRAND_NAMES.map((nm,i)=>({id:i+1,slug:String(nm).toLocaleLowerCase(TR).replace(/\s+/g,"-"),name:nm,count:"—"}))}
    else{brandPrefix="Hazır";COMPEL_BRANDS_CACHE?.length?BRANDS=COMPEL_BRANDS_CACHE:await initBrands()}
...
async function initBrands(){
  ...
  try{const data=await loadBrands(API_BASE);COMPEL_BRANDS_CACHE=data;ACTIVE_SUPPLIER===SUPPLIERS.COMPEL&&(BRANDS=data)}
  catch(e){console.error(e);if(ACTIVE_SUPPLIER===SUPPLIERS.COMPEL){const el2=$("brandStatus");el2&&(el2.textContent="Markalar yüklenemedi (API).",el2.title=el2.textContent)}}
...
const depot=createDepot({
  ...
  onDepotLoaded:async()=>{
    ...
    }catch(err){console.error(err);setStatus(String(err?.message||err),"bad");alert(String(err?.message||err))}
  }
});
...
function rebuildTsoftOkSupByBrand(){
  ...
  for(const row of(R||[])){
    ...
    const br=row._bn||normBrand(row["Marka"]||""),sup=T(row["Ürün Kodu (T-Soft)"]||"");
    if(!br||!sup)continue;
...
function buildUnifiedUnmatched({Uc,Ut,Ud}){
  const g=new Map(),getGrp=(brNorm,brandDisp)=>{
    ...
    const grp=g.get(k);
    brandDisp&&(!grp.brandDisp||grp.brandDisp===grp.brNorm)&&(grp.brandDisp=brandDisp);
    return grp
  };
...
function compactTsoftCSV(L2all,C2){
  ...
  const rows=(L2all||[]).map(r=>({"Web Servis Kodu":T(r[C2.ws]||""),"Ürün Adı":T(r[C2.urunAdi]||""),"Tedarikçi Ürün Kodu":T(r[C2.sup]||""),"Barkod":T(r[C2.barkod]||""),"Stok":T(r[C2.stok]||""),"Marka":T(r[C2.marka]||""),"SEO Link":T(r[C2.seo]||""),"Aktif":C2.aktif?T(r[C2.aktif]||""):""}));
...
async function generate(){
  ...
  try{
    ...
    if(needDaily){
      ...
      const want=[];DAILY_SELECTED.tsoft&&want.push("tsoft");DAILY_SELECTED.aide&&want.push("aide");
...
function resetAll(){
  ...
  const f2=$("f2");f2&&(f2.value="");const n2=$("n2");n2&&(n2.textContent="Yükle",n2.title="Yükle");

  TSOFT_OK_SUP_BY_BRAND=new Map();COMPEL_BRANDS_NORM=new Set();
  DAILY_SELECTED={tsoft:"",aide:""};DAILY_READ_CACHE={date:"",pass:""};DAILY_SAVE_CRED=null;
...
const goBtn2=$("go");goBtn2&&(goBtn2.onclick=handleGo);

/* aide save checkbox */
$("aideSaveToday")?.addEventListener("change",e=>{const cb=e.target;cb.checked&&!ensureSaveCredOrCancel()&&(cb.checked=false)});
